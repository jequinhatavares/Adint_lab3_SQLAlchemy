...
class Book(Base):
    ...
    publishingDate = Column(Date, nullable=True)
    ...
    def __repr__(self):
        return "<Book(isbn ='%s', title = %s, author_id = %d, editora = %s, publishingDate= %s)>" % (
            self.isbn, self.title, self.author_id, self.publisher, str(self.publishingDate))
...
def getBooksFromDates(date1, date2):
    books = session.query(Book).filter(Book.publishingDate > date1, Book.publishingDate < date2).all()
    return books
...
def newBook(authorID, isbn, title, publisher, publishing_Date):
...
if __name__ == "__main__":
    ...
    print("Books between dates")
    books_between_dates = getBooksFromDates(datetime.date(day=1, month=2, year=2000),datetime.date(day=1, month=2, year=2002))
    print(books_between_dates)
